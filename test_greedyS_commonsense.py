...
import argparse
from data_converter import convert_dataset,convert_cnn_dataset,convert_c4_dataset_eval,convert_yahoo_dataset_eval,convert_ag_dataset_eval
import argparse
from GreedySTree_commonsense import GreedySTree
...
def simulation_greedy_with_tree_fast(target_model : GraphInferenceEngineTG, draft_model: GraphInferenceEngine, dataloader: DataLoader, T=0.6, top_p=0.9, 
    ...
    with torch.no_grad():
        output_file = "output_openwebtext_10.txt"
...
if args.dataset == 'openwebtext':
    # Load the OpenWebText dataset from disk and select the specified range
    tokenized_dataset_eval = load_from_disk("dataset/openwebtext_eval").select(list(range(args.start, args.end)))

elif args.dataset == 'cnn':
    # Convert the CNN dataset and select the specified range
    tokenized_dataset_eval = convert_cnn_dataset(tokenizer=tokenizer, seq_len=256).select(list(range(args.start, args.end)))

elif args.dataset == 'c4':
    # Convert the Yahoo dataset and select the specified range
    tokenized_dataset_eval = convert_yahoo_dataset_eval(tokenizer=tokenizer, seq_len=256).select(list(range(args.start, args.end)))

else:
    # Default case: Convert a dataset from a file path
