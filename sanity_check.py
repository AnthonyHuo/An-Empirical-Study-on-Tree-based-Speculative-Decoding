import torch
from Engine import InferenceEngineTG
...
engine = InferenceEngineTG(max_length=MAX_LENGTH, model_name_or_path=MODEL_NAME, dtype=DTYPE, device=DEVICE)

logits = engine.model_run(input_ids=input_ids, storage_ids=storage_ids, position_ids=position_ids, attention_mask=attn_mask[None, None,:,:len(storage_ids)])
...
logits = engine.model_run(input_ids=extra_input_ids, storage_ids=extra_storage_ids, position_ids=extra_position_ids, attention_mask=extra_attn_mask[None, None,:,:len(storage_ids)+len(extra_storage_ids)])
