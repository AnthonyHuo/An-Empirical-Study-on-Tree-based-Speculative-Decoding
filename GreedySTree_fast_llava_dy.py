...
def generate_parent_indices(root):
    ...
    nodes = [root]
    parent_indices = [-1] * 128  # Initialize the parent indices list with -1
    node_index = 0
...
class GreedySTree(Tree):
    def __init__(self, 
        ...
        self.mask = torch.eye(127, dtype=torch.bool).to(self.device)
        self.seq_to_use = list(range(self.max_length))
        self.attn_engine = AttentionMaskEngine(device=self.device, dtype=self.dtype)
        self.parent_nodes = torch.arange(1, 128, device=self.device)
        self.parent_nodes_0 = torch.arange(1, device=self.device)
    # def generate_attention_mask(self, parent_indices, num_nodes=127):
    #     """Generate an attention mask of size (127, 127) where the node and its path to root are marked with 1."""
    #     self.mask[0][0] = 1
    ...
    def prune_tree(self,logits, parent_indices, depths, values, cumulative_logits, keep=127,updated_tensor=0):
        """Prune the tree to keep up to 'keep' nodes based on unique cumulative logits, prioritizing uniqueness."""
        # Sort nodes based on cumulative logits to prioritize higher values
        # t1 = time.time()
    ...
    def update_tree_with_logits(self,logits, parent_indices, depths, values, cumulative_logits, new_logits, grow_step, updated_tensor):
        ...
        if grow_step != 0:
            ...
            new_tokens_values, new_tokens_set = new_logits.topk(k=127)
        # t2 = time.time()
        # time_taken = t2 - t1
        ...
        pruned_logits, pruned_parent_indices, pruned_depths, pruned_values, pruned_cumulative_logits,updated_tensor = self.prune_tree(
            updated_logits, updated_parent_indices, updated_depths, updated_values, updated_cumulative_logits, keep=128, updated_tensor = updated_tensor
        )
        # t21 = time.time()
        # time_taken9 = t21 - t20
    ...
    def collective_grow_dynamic(self, benchmark=False, grow_step = None):
        ...
        self.tokens[self.num_nodes: self.num_nodes + 127] = self.values[1:]
